---
title: Nums v1
description: My first version of Nums — a two-player number strategy game, built with plain HTML, CSS and JavaScript as part of the v1 homepage.
status: completed
startDate: 2017-06-25
lastUpdated: 2019-10-16
image: nums-v1.png
url: https://ajur.github.io/old-nums/
links:
  - https://github.com/ajur/ajur.github.io/tree/master/old-nums
tags:
  - gamedev
---

My first version of Nums — a two-player number strategy game, built with plain HTML, CSS and JavaScript as part of the v1 homepage.

Built with plain HTML, CSS and JavaScript as part of the v1 personal homepage. Sources are preserved in the `old-nums` directory of the v1 homepage branch.

## Tech Stack

- HTML / CSS / JavaScript
